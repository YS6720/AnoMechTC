...
public sealed class TopP5DeltaScenario : IScenario
{
    ...
    {
        // The scenario instance survives retry; retired actors must not affect the new party.
        tethersShort.Clear();
        tethersLong.Clear();
        world = worldParam;
        ...
        world.Events.Add(65.1f, () =>
        {
            CheckTethersExpired(tethersLong);
            world.CompleteScenario();
        });             // tethers expire t=30.2+36=66.2
    }
    ...
    private void OnTetherFailed(SimTether tether)
    {
        ...
        tether.Resolved = true;
        party.WipeAllPlayers("Hello, World 連線失敗（全隊滅團）");
    ...
    {
        if (IsDamageLethal(player, magic: true, comeRuin: 3)) { player.Die("Hello, World 連線斷裂"); return; }
    ...
    private void ResolvePunchExplosions()
    {
        ...
        if (state.PunchExplosionUnmitigated)
        {
            ...
            party.WipeAllPlayers("火箭拳擊—未減傷爆炸");
        }
    }
