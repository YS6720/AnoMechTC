...
public class DamageSolver
{
        ...
        bool killTargets = true, bool reportsMechanicHit = true)
    {
        ...
        foreach (var target in targets)
        {
            ...
            if (targets.Count < stackMinTargets)
            {
                ...
                if (killTargets)
                    target.Die($"{ActionLookup.Name(actionId)}（分攤人數不足：{targets.Count}/{stackMinTargets}）");
            }
    ...
    private bool CheckLethal(uint actionId, SimCharacter target, HashSet<DamageType> damageTypes, bool killTarget)
    {
        ...
        {
            if (killTarget)  target.Die($"受到{ActionLookup.Name(actionId)}致命傷害");
            return true;
        }
        else if (IsLethal(target, damageTypes))
        {
            if (killTarget) target.Die($"{ActionLookup.Name(actionId)}（帶有易傷狀態）");
            return true;
        }
        ...
        {
            if(killTarget) target.Die($"{ActionLookup.Name(actionId)}（非坦克承受死刑）");
            return true;
        }
        else
        {
