...
public unsafe class SimNpc : SimCharacter
{
    ...
    private int index;
    private readonly bool requiresDrawObject;
    private bool pendingDraw;
    private float pendingDrawSeconds;
    private bool drawWaitReported;
    ...
    protected SimNpc(int index, Coordinates coordinates, bool requiresDrawObject = true) : base(coordinates)
    {
        this.index = index;
        this.requiresDrawObject = requiresDrawObject;
        pendingDraw = index != InvalidIndex && requiresDrawObject;
    }

    public override bool IsActive => index != InvalidIndex && BattleCharaPtr != null;
    
    // Drawable actors must finish their async model load before Prepare commits.
    // ModelChara.Type 0 actors have no model or draw lifecycle: only their native
    // actor must exist. Hidden actors with real models still take the full gate.
    internal bool IsReadyForNetwork
    {
        get
        {
            var obj = BattleCharaPtr;
            return IsActive && !pendingDraw && obj != null
                && (!requiresDrawObject || (obj->DrawObject != null && obj->IsReadyToDraw()));
        }
    }

    public virtual void SetModelState(byte value)
    {
    ...
    public void SetModeAttributeFlags(byte value)
    {
        var chara = BattleCharaPtr;
        if (chara == null) return;
        // 台服 CS 無此欄位 ⇒ 目前為 no-op（見 Compat/ModelContainerCompat.cs）。
        // 沒套用就不重建模型——省掉一次無意義的 DisableDraw/EnableDraw 閃爍。
        if (!ModelContainerCompat.TrySetModeAttributeFlags(chara, value)) return;
        ReloadModel();
    }
    ...
    private void ReloadModel()
    {
        ...
        obj->DisableDraw();
        pendingDraw = true;
        pendingDrawSeconds = 0f;
        drawWaitReported = false;
    }
    ...
    public override void Tick(float deltaSeconds)
    {
        ...
        if (pendingDraw)
        {
            ...
            else if (obj->IsReadyToDraw())
            {
                ...
                pendingDraw = false;
                if (drawWaitReported)
                    Plugin.Log.Information($"[AnoMech 模型] CO={index} EntityId={obj->EntityId:X8} 已可繪製，等待 {pendingDrawSeconds:F1} 秒。");
            }
            else
            {
                pendingDrawSeconds += deltaSeconds;
                // 每輪載入只報一次；不用逐幀同步落盤，也不強制繪製未就緒模型。
                if (!drawWaitReported && pendingDrawSeconds >= 5f)
                {
                    drawWaitReported = true;
                    Plugin.Log.Warning($"[AnoMech 模型] CO={index} EntityId={obj->EntityId:X8} 等待繪製超過 5 秒；請查模型資源，這不是副本可用認證。");
                }
            }
        }
    }
