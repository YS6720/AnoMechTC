...
internal readonly record struct SimOmenPart(
    int Part, string ResourcePath, Vector3 Position, Quaternion Rotation, Vector3 Scale);
...
public sealed unsafe class SimOmen : ISimObject
{
    ...
    // Action-derived omens are already produced by the native cast packet and
    // must not be replayed as an independent static visual. Explicit SpawnOmen
    // instances are the only static visuals represented on the wire.
    private readonly bool networkVisual;
    private string? primaryResourcePath;
    ...
    internal SimOmen(Coordinates coordinates, string omenPath, Placement placement, Vector3 scale,
        float? durationSeconds = null, Quaternion? rotation = null)
    {
        this.coordinates = coordinates;
        networkVisual = true;
        remaining = durationSeconds;
        ...
        primaryResourcePath = resolved;
        primaryPosition = placement.Position;
        primaryRotation = rotation ?? Quaternion.CreateFromYawPitchRoll(placement.Rotation, 0f, 0f);
        primaryScale = scale;
        primary = VfxFunctions.SpawnStaticVfx(resolved, coordinates.ToGlobal(placement), scale, rotation);
    }
    internal void UpdateRecordedTransform(Vector3 position, Quaternion rotation, Vector3 scale)
    {
        primaryPosition = position;
        primaryRotation = rotation;
        primaryScale = scale;
        VfxFunctions.SetStaticVfxTransform(primary, coordinates.ToGlobal(position), rotation, scale);
    }

    internal bool HasNetworkVisual => networkVisual;
    internal bool IsNetworkVisualReady
        => !networkVisual || primary != null;

    internal SimOmenPart? NetworkPart => networkVisual && primary != null && primaryResourcePath is { } path
        ? new SimOmenPart(0, path, primaryPosition, primaryRotation, primaryScale)
        : null;
