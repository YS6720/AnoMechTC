...
internal static unsafe class VfxFunctions
{
    ...
    public static VfxObject* SpawnStaticVfx(string path, Placement placement, Vector3 scale, Quaternion? rotation = null)
    {
        ...
        {
            // 台服 CS 無 VfxObject.Create ⇒ 走自行 signature-scan 的函式指標
            vfx = VfxObjectPointers.Create(pathPtr, poolPtr);
        }
        if (vfx == null) return null;

        SetStaticVfxTransform(vfx, placement.Position,
            rotation ?? Quaternion.CreateFromYawPitchRoll(placement.Rotation, 0f, 0f), scale);
        // SomeFlags 與 Update 在台服 CS 未宣告 ⇒ 走 Compat／Pointers（位移與 sig 皆已離線驗證）
        VfxObjectCompat.SomeFlags(vfx) &= 0xF7;   // clear flag that sometimes hides the vfx

        VfxObjectPointers.Update(vfx, 0f, -1);
        return vfx;
    }

    internal static void SetStaticVfxTransform(VfxObject* vfx, Vector3 position, Quaternion rotation, Vector3 scale)
    {
        if (vfx == null) return;
    ...
    {
        if (vfx == null) return;
        VfxObjectCompat.CleanupRender(vfx);   // 台服 CS 未宣告此 virtual ⇒ 走 vtable[1]
    }
    ...
    public static string? LockonVfxIconName(uint lockonId)
    {
        ...
        // 台服 Lumina 尚未替 Lockon 的欄位命名：Unknown0 ＝ 國際服的 IconName。
        // 依據：兩版欄位順序相同（RowId, [此欄], Unknown1），國際服該位置名為 IconName。
