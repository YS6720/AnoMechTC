...
internal sealed unsafe class PartyHud
{
    ...
    private readonly PartyRole?[] slotRoles = new PartyRole?[MaxSlots];
    private byte lastWrittenMemberCount;
    private bool hasLastWritten;
    private bool displayActive;

    // Source-row mapping used only by the display adapter. It is deliberately
    // derived from the exact slot writes below; no consumer should infer that
    // a native row number equals a PartyRole enum value.
    public IReadOnlyList<PartyRole?> SlotRoles => slotRoles;
    public bool IsDisplayActive => displayActive;

    public void Refresh(SimParty party)
    {
        if (party.AllMembers().Count() < 2)
        {
        ...
        foreach (var member in party.AllMembers())
        {
            ...
            slotRoles[index] = partyMember.Role;
            // Name comes from the sim party's projection when the slot has one (a
            // connected player's alias); otherwise the character's own name is copied
            // verbatim. The local player's real GameObject name is never rewritten —
            // only this row's copy of it is replaced.
            WriteSlot(ref grp.PartyMembers[index], bc, party.EncodedDisplayName(partyMember.Role));
    ...
    private static void WriteSlot(ref GroupPartyMember slot, BattleChara* bc, ReadOnlySpan<byte> displayName)
    {
